...
public class DoctorJpaController implements Serializable {
   ...
   //agrege adicional 
     public List<Doctor> findDoctoresByEspecialidad(String especialidad) {
        ...
        try {
            // Crear una consulta con JPQL
            TypedQuery<Doctor> query = em.createQuery("SELECT d FROM Doctor d WHERE d.especialidad = :especialidad", Doctor.class);
            query.setParameter("especialidad", especialidad);
            return query.getResultList(); // Devuelve la lista de doctores encontrados
        } finally {
            em.close();
        }
    }
